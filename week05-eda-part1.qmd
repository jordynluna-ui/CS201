...
skim_data <- skimr::skim(pokedata) # creates the skim data
view(skim_data)
...
Answer: there are pokemon with a lot higher sp_atk than 65. It suggests the distribution is skewed to the right — a tail of unusually high values is pulling the mean up. The median stays put because it only cares about position, not magnitude.
...
- you have the middle line, the whiskers, and the box.
