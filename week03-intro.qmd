...
- What are the variables you see?
  ...
  - model, mpg, cyl, disp, hp, drat, wt, qsec, vs, am, gear, and carb.
...
mean(mtcars) # Wrong
meant(mpg) # Wrong Again

mean(mtcars$mpg) # $ lets you select a variable/column within the dataframe
```
...
mtcars <- mtcars |> 
  mutate(am_f = factor(am, labels = c("auto", "manuel")))
