...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) + 
  geom_histogram(alpha = 0.75) + # creating a histogram, alpha changes transparency
  scale_fill_manual(values = c("darkorange", "darkorchid", "cyan4"), name = "Penguin Species") + # manual colors
  theme_bw() + # background colors
...
```{r, message=FALSE}
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) + # set up line for ggplot
  geom_histogram(alpha = 0.75) + # creating a histogram, alpha changes transparency
  scale_fill_viridis_d(option = "plasma", name = "Penguin Species") + # color blind set colors
  theme_bw() + # background colors
  labs(title = "Palmer Penguins' Flipper Length",
       subtitle = "Three species observed on three islands in the Palmer Archipelago",
       x = "Flipper Length (mm)",
...
ggplot(data = penguins_data, aes(x = species, y = flipper_length_mm, fill = species)) + 
  geom_boxplot() +
  geom_jitter(size = 0.5) + # points to see all of the species, size of points
  theme_bw() +
  scale_fill_manual(values = c("darkorange", "darkorchid", "cyan4"), name = "Penguin Species") + # manual colors
  theme_bw() + # background colors
  labs(title = "Palmer Penguins' Flipper Length",
       ...
       x = "Flipper Length (mm)",
       y = "Number of Penguins") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none") + # text on the x axis, hjust = horizontal adjustment, 
  coord_flip()
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, y = body_mass_g, color = species, shape = island)) + # color instead of fill for points, shape = of the points
  geom_point() + # scatterplot
  scale_shape_manual(values = c(9,15,25)) # changing the scale of something
...
plotname <- ggplot(data = penguins_data, aes(x = culmen_depth_mm, fill = species)) +
  ...
  labs(title = "Palmer Penguins",
       subtitle = "Culmen depth of Palmer Penguins",
       x = "Culmen depth (mm)",
       y = "Number of Penguins")
...
ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  geom_point() +
  theme_bw() +
  scale_colour_viridis_d(option = "mako", name = "Species") +
  labs(title = "Palmer Penguins",
       subtitle = "The culmen depth and body mass of each species",
       x = "Culmen Depth (mm)",
       y = "Body Mass (g)")
...
```{r, message=FALSE}
Palmer_Penguins <- ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  geom_point() +
  theme_bw() +
  scale_colour_viridis_d(option = "mako", name = "Species") +
  labs(title = "Palmer Penguins",
